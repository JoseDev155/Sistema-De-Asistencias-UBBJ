...
# RUTAS DE CICLOS ACADÉMICOS
@academic_cycle_controller.get("/academic-cycles", tags=["academic-cycles"],
                               description="Endpoint para obtener todos los ciclos académicos del sistema",
                               response_model=list[AcademicCycleResponse])
async def get_academic_cycles(db: Session = Depends(get_db)) -> dict[str, object]:
...
@academic_cycle_controller.get("/academic-cycles/{id}", tags=["academic-cycles"],
                               description="Endpoint para obtener un ciclo académico específico por su ID",
                               response_model=AcademicCycleResponse)
async def get_academic_cycle(id: int, db: Session = Depends(get_db)) -> dict[str, object]:
...
@academic_cycle_controller.get("/academic-cycles/search/{name}", tags=["academic-cycles"],
                               description="Endpoint para obtener un ciclo académico específico por su nombre",
                               response_model=AcademicCycleResponse)
async def get_academic_cycle_by_name(name: str, db: Session = Depends(get_db)) -> dict[str, object]:
...
@academic_cycle_controller.post("/academic-cycles", tags=["academic-cycles"],
                                description="Endpoint para crear un nuevo ciclo académico en el sistema",
                                response_model=AcademicCycleResponse)
async def create_academic_cycle(
    cycle: AcademicCycleCreate, db: Session = Depends(get_db)
...
@academic_cycle_controller.put("/academic-cycles/{id}", tags=["academic-cycles"],
                               description="Endpoint para actualizar un ciclo académico específico por su ID",
                               response_model=AcademicCycleResponse)
async def update_academic_cycle(
    id: int, cycle: AcademicCycleUpdate, db: Session = Depends(get_db)
    ) -> dict[str, AcademicCycleResponse] | dict[str, object]:
    try:
...
@academic_cycle_controller.delete("/academic-cycles/{id}", tags=["academic-cycles"],
                                  description="Endpoint para eliminar un ciclo académico específico por su ID")
async def delete_academic_cycle(id: int, db: Session = Depends(get_db)) -> dict[str, object]:
