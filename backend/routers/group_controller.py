...
@group_controller.get("/groups", tags=["groups"],
                     description="Endpoint para obtener todos los grupos del sistema",
                     response_model=list[GroupResponse])
async def get_groups(db: Session = Depends(get_db)) -> dict[str, object]:
...
@group_controller.get("/groups/{id}", tags=["groups"],
                     description="Endpoint para obtener un grupo específico por su ID",
                     response_model=GroupResponse)
async def get_group(id: str, db: Session = Depends(get_db)) -> dict[str, object]:
...
@group_controller.get("/groups/search/{name}", tags=["groups"],
                     description="Endpoint para obtener un grupo específico por su nombre",
                     response_model=GroupResponse)
async def get_group_by_name(name: str, db: Session = Depends(get_db)) -> dict[str, object]:
...
@group_controller.post("/groups", tags=["groups"],
                      description="Endpoint para crear un nuevo grupo en el sistema",
                      response_model=GroupResponse)
async def create_group(
    group: GroupCreate, db: Session = Depends(get_db)
...
@group_controller.put("/groups/{id}", tags=["groups"],
                     description="Endpoint para actualizar un grupo específico por su ID",
                     response_model=GroupResponse)
async def update_group(
    id: str, group: GroupUpdate, db: Session = Depends(get_db)
    ) -> dict[str, GroupResponse] | dict[str, object]:
    try:
...
@group_controller.delete("/groups/{id}", tags=["groups"],
                        description="Endpoint para eliminar un grupo específico por su ID")
async def delete_group(id: str, db: Session = Depends(get_db)) -> dict[str, object]:
