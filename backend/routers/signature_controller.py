...
@signature_controller.get("/signatures", tags=["signatures"],
                         description="Endpoint para obtener todas las asignaturas del sistema",
                         response_model=list[SignatureResponse])
async def get_signatures(db: Session = Depends(get_db)) -> dict[str, object]:
...
@signature_controller.get("/signatures/{id}", tags=["signatures"],
                         description="Endpoint para obtener una asignatura específica por su ID",
                         response_model=SignatureResponse)
async def get_signature(id: str, db: Session = Depends(get_db)) -> dict[str, object]:
...
@signature_controller.get("/signatures/search/{name}", tags=["signatures"],
                         description="Endpoint para obtener una asignatura específica por su nombre",
                         response_model=SignatureResponse)
async def get_signature_by_name(name: str, db: Session = Depends(get_db)) -> dict[str, object]:
...
@signature_controller.post("/signatures", tags=["signatures"],
                          description="Endpoint para crear una nueva asignatura en el sistema",
                          response_model=SignatureResponse)
async def create_signature(
    signature: SignatureCreate, db: Session = Depends(get_db)
...
@signature_controller.put("/signatures/{id}", tags=["signatures"],
                         description="Endpoint para actualizar una asignatura específica por su ID",
                         response_model=SignatureResponse)
async def update_signature(
    id: str, signature: SignatureUpdate, db: Session = Depends(get_db)
    ) -> dict[str, SignatureResponse] | dict[str, object]:
    try:
...
@signature_controller.delete("/signatures/{id}", tags=["signatures"],
                            description="Endpoint para eliminar una asignatura específica por su ID")
async def delete_signature(id: str, db: Session = Depends(get_db)) -> dict[str, object]:
